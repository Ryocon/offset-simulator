    ...
    <form  onSubmit={handleSubmit}>
      {formValues.map((element, index) => (
        <div className="form-inline" key={index}>
          <label>Month</label>
          <input type="month" name="month" required={true} value={element.month || ""} onChange={e => handleChange(index, e)} />
          <label>Tree Amount</label>
          <input type="number" name="number" minLength={0} maxLength={55} value={element.number || ""} onChange={e => handleChange(index, e)} />
          {
            index ? 
